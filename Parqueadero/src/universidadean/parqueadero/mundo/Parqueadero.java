...
public class Parqueadero {
    ...
    public String metodo1() {
        String placaMaximoTiempo="";
        int maximoTiempo=0;
        for(int i=0;i<puestos.length;i++){
            if(puestos[i].darCarro()!=null){
                if(puestos[i].darCarro().darTiempoEnParqueadero(horaActual)>maximoTiempo){
                    maximoTiempo=puestos[i].darCarro().darTiempoEnParqueadero(horaActual);
                    placaMaximoTiempo=puestos[i].darCarro().darPlaca();
                }
            }
        }
        return placaMaximoTiempo;
    }
